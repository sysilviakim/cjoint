...
amce_mod <- function(varlist, data) {
  myformula <- as.formula(
    paste("Chosen_Immigrant ~ ", paste(varlist, collapse = "+"))
  )
...
amce_additive_mod_cluster_weights <- function(varlist,
                                              data,
                                              clustervar,
                                              weightvar) {
  myformula <- as.formula(
...
mod2 <- amce_additive_mod_cluster(
  varlist1,
  data = immigrationconjoint,
  clustervar = "CaseID"
)
mod3 <- amce_additive_mod_cluster_weights(
  varlist1,
  data = immigrationconjoint,
  clustervar = "CaseID",
  weightvar = "weights"
...
test_that("Warnings are issued.", {
  ...
  expect_warning(
    amce_mod(varlist1, immigrationconjoint),
    paste0(
...
test_that("Error for duplicate variables.", {
  expect_error(
    amce_additive_mod(varlist3, tempdata),
    paste0(
      "Error: Variable names must be unique when ",
      "whitespace and meta-characters are removed. Please rename."
    )
  )
})
...
test_that("error missing variables", {
  ...
  expect_error(
    amce_additive_mod(
      varlist1, (immigrationconjoint[, !names(immigrationconjoint) %in% var])
    ),
...
test_that("warning change to factor", {
  ...
  expect_warning(
    amce(
      Chosen_Immigrant ~ Education + Gender,
      respondent.varying = NULL,
      data = temp_num,
      cluster = FALSE
    ),
...
test_that("error missing values in data", {
  ...
  missing <- sample(
    x = 1:nrow(tempdf), size = .1 * nrow(tempdf), replace = FALSE
  )
...
test_that("Error: supplied baseline is not a level.", {
  ...
  expect_error(
    amce(
      Chosen_Immigrant ~ Gender + Education +
        `Country of Origin` + `Country of Origin` + Job,
      data = immigrationconjoint,
      design = immigrationdesign,
      respondent.varying = NULL,
      subset = !is.na(immigrationconjoint$ethnocentrism),
...
test_that("Error: weights not in data.", {
  expect_error(
    amce(
      Chosen_Immigrant ~ Gender + Education + `Country of Origin` + Job,
      data = immigrationconjoint,
      design = immigrationdesign,
      respondent.varying = NULL,
      respondent.id = NULL,
...
test_that("Error: respondent.varying var not in the formula.", {
  expect_error(
    amce(
      Chosen_Immigrant ~ Gender + `Country of Origin` + Job,
      data = immigrationconjoint,
      design = immigrationdesign,
      respondent.varying = "Education",
      respondent.id = "CaseID",
      cluster = TRUE, na.ignore = TRUE, weights = NULL, baselines = NULL
    ),
    paste0(
      "Error: Education is specified in respondent.varying, ",
      "but is not in the formula."
    )
  )
})
...
test_that("Error: not in 'design' object.", {
  ...
  attribute_list[["Country of Origin"]] <- c(
    "Germany", "France", "Mexico",
    "Philippines", "Poland", "India",
    "China", "Sudan", "Somalia", "Iraq"
  )
  temp_design <- makeDesign(
    type = "constraints", attribute.levels = attribute_list
  )

  expect_error(
    amce(
      Chosen_Immigrant ~ Gender + Education + Job,
      data = immigrationconjoint,
      design = temp_design,
      respondent.varying = "Education",
      respondent.id = "CaseID",
...
test_that("Error: factor level of attribute not in 'design' object.", {
  ...
  expect_error(
    amce(
      Chosen_Immigrant ~ Gender + Education + `Country of Origin`,
      data = immigrationconjoint,
      design = temp_design,
      respondent.varying = "Education",
      respondent.id = "CaseID",
    ...
    paste0(
      "Error: factor level China of attribute ",
      "CountryofOrigin not in 'design' object."
    )
  )
})
...
test_that(
  paste0(
    "Error:'design' must be a valid character ",
    "string ('uniform') or a conjointDesign object."
  ),
  {
    expect_error(
      amce(
      ...
      paste0(
        "Design object must be a valid character ",
        "string 'uniform' or a conjointDesign object."
      )
    )
  }
...
test_that("Error: levels of variables", {
  ...
  expect_error(
    amce(
      Chosen_Immigrant ~ Gender + Education + `Language Skills` +
    ...
    paste0(
      "Error: levels of variable Education are not unique ",
  ...
  expect_error(
    amce(
      Chosen_Immigrant ~ Gender + `Language Skills` +
        `Country of Origin` + Job + `Job Experience` + `Job Plans` +
        `Reason for Application` + `Prior Entry`,
      data = df,
      cluster = TRUE,
    ...
    paste0(
      "Error: levels of variable Education used as a dependency, ",
