...
test_that("Incorrect CI returns error.", {
  expect_output(
    plot(results, ci = 1.5), "Invalid confidence interval: Defaulting to 95%."
  )
})

test_that("Valid plot display option.", {
  expect_error(
    plot(results, plot.display = "other"),
    "Error: plot.display must be once of: all, unconditional, interaction"
  )
})
