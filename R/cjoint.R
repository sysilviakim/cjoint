...
plot.amce <- function(x, main = "",
  ...
  if (any(!facet.names %in% names(facet.levels))) {
    for (facet.name in facet.names[!facet.names %in% names(facet.levels)]) {
      ...
      if (facet.name %in% names(amce_obj$baselines)) {
        ...
        # names from user input
        fac.levs <- c(
          paste(facet.name, amce_obj$baselines[[facet.name]], sep = ""), fac.levs
        )
  ...
  if (plot.display != "interaction") {
    ...
    if (plot.display == "all" && !is.null(facet.names)) {
      attr_remove <- c()
      for (
        facet.name in facet.names[!is.element(facet.names, amce_obj$respondent.varying)]) {
    ...
    for (i in 1:length(raw_attributes)) {
      ...
      d_head <- data.frame(
        ...
        lower = NA,
        var = attr_name,
        printvar = paste(print_attr_name, ":", sep = ""),
        group = "<NA>",
      ...
      for (j in 1:length(raw_levels[[attr_name]])) {
        ...
        if (j == 1) {
          ...
          d_lev <- data.frame(
            ...
            lower = NA,
            var = level_name,
            printvar = paste("   ", print_level_name, sep = ""),
  ...
  if (plot.display != "unconditional" & !is.null(facet.names)) {
    ...
    for (facet.name in facet.names) {
      ...
      if (length(all_mod) == 0) {
        stop(
          paste(c(
            "Error: Facet variable", facet.name, "not interacted with profile attributes"
          ), collapse = " ")
        )
      }
      # just unique ones
      ...
      for (k in facet.start:length(facet.levels[[facet.name]])) {
        ...
        if (is.element(facet.name, names(amce_obj$estimates))) {
          # if ACIE
          print_facet_level <- paste(c(
            "ACIE",
            paste(c(print_facet_name, names(facet.levels[[facet.name]])[k]), collapse = " = ")
          ), collapse = "\n")
        } else {
          # if conditional on respondent varying
          print_facet_level <- paste(c(
      ...
      theme_grey(base_size = base_size, base_family = base_family) %+replace%
        theme(
          ...
          axis.text.y = element_text(
            size = base_size, colour = text.color, hjust = 0, vjust = .5, family = font.family
          ),
          axis.ticks = element_line(colour = "grey50"),
          axis.title.y = element_text(
            size = base_size, angle = 90, vjust = .01, hjust = .1, family = font.family
          ),
          plot.title = element_text(
            face = "bold", family = font.family
    ...
    theme_bw1 <- function(base_size = text.size, base_family = "") {
      theme_grey(base_size = base_size, base_family = base_family) %+replace%
        theme(
          ...
          axis.text.y = element_text(size = base_size, colour = text.color, hjust = 0, vjust = .5, family = font.family),
          axis.ticks = element_line(colour = "grey50"),
          axis.title.y = element_text(size = base_size, angle = 90, vjust = .01, hjust = .1, family = font.family),
  ...
  if (length(covariate.values) > 1) {
    ...
    for (this.var in names(covariate.values)) {
      resp.message <- paste(c(
        resp.message, " For AMCE and ACIE conditional on ", this.var, ", "
      ), collapse = "")
      ...
      for (var in other.vars) {
        other.levels <- c(
          other.levels,
          paste(c(var, " will be held at level \"", names(covariate.values[[var]])[1], "\""), collapse = "")
        )
      }
...
read.qualtrics <- function(filename,
    ...
    if (is.null(unique(qualtrics_data[, attr_column]))) {
      stop(
        paste(
          ...
          var_names[attr_column], " has no attribute names - recommend deleting this column"
        )
      )
  ...
  if (sum(grepl("[\\'\"]", unique(all_attr))) > 0) {
    stop(
      paste(
        "Error, attribute name has special characters. Some special characters are reserved for this function (if cjoint>v2.0.6) for the purpose of efficiency. If you still want to display special character in your plot, use the argument attribute.names in the plot function. See manual for more details."
      )
    )
  } else {
  ...
  if (is.null(ranks)) {
    for (i in num_tasks) {
      ...
      for (j in num_profiles) {
        temp.cmd <- paste0(
          "out_data_dataset$'selected_", j, "-", i, "'<-", "ifelse(temp.selected==j,1,0)"
        )
        eval(parse(text = temp.cmd))
        temp.cmd <- paste0(
          "out_data_dataset$'selected_", j, "-", i, "'[temp.selected", "=='']", "<-", "''"
        )
        eval(parse(text = temp.cmd))
      }
    }
    ...
    for (i in num_tasks) {
      for (j in num_profiles) {
        temp.cmd <- paste0(
          "out_data_dataset$'selected_", j, "-", i, "'<-", "qualtrics_data[,", ranks_col[(i - 1) * length(num_profiles) + j], "]"
        )
        eval(parse(text = temp.cmd))
      }
    }
