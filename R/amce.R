...
amce <- function(formula,
  ...
  if ("conjointDesign" %in% class(design)) {
    ...
    depend_to_check <-
      depend_to_check[!depend_to_check %in% var_to_check_levels]
  ...
  ############## loop over unique profile vars only (AMCE and ACIE); 
  ############## interactions etc. below
    ...
    for (j in 1:nrow(levels)) {
      ...
      if (!is.na(initial_beta) & length(substrings) > 1) {
        for (effect1 in substrings) {
          ...
          # loop over other profile-varying vars 
          # in interaction to subset further
      ...
      # If initial_beta and initial_variance are not NA 
      # (are valid level combination)
